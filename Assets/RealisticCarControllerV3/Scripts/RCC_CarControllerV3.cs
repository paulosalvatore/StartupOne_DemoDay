...
public class RCC_CarControllerV3 : MonoBehaviour
{
	...
	{ public Vector3[] meshVerts; }        // Struct for Original Mesh Verticies positions.
	...
	[Range(.05f, .5f)]
	public float ABSThreshold = .35f;

	[Range(.05f, .5f)]
	public float TCSThreshold = .25f;

	[Range(0f, 1f)]
	public float TCSStrength = 1f;

	[Range(.05f, .5f)]
	public float ESPThreshold = .25f;

	[Range(.1f, 1f)]
	public float ESPStrength = .5f;
	...
	private void Inputs()
	{
		switch (RCCSettings.controllerType)
		{
			case RCC_Settings.ControllerType.Keyboard:
				...
				gasInput = Mathf.Min(1, Input.GetAxis(RCCSettings.verticalInput) * 1.3f);
				brakeInput = Mathf.Clamp01(-Mathf.Min(1, Input.GetAxis(RCCSettings.verticalInput) * 1.3f));
	...
	private void Engine()
	{
		...
		#endregion Wheel Type Motor Torque.
	}
